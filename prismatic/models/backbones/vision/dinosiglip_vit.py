...
class DinoSigLIPViTBackbone(VisionBackbone):
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        Args:
            pixel_values: dict with keys "dino" and "siglip", each of shape (B, N, C, H, W)
        Returns:
            visual_tokens: (B, N * num_patches, embed_dim)
        """
        dino_input = pixel_values["dino"]    # (B, N, C, H, W)
        siglip_input = pixel_values["siglip"]
    
        B, N = dino_input.shape[:2]
        # 合并 batch 和视角维度
        dino_flat = dino_input.view(B * N, *dino_input.shape[2:])   # (B*N, C, H, W)
        siglip_flat = siglip_input.view(B * N, *siglip_input.shape[2:])
    
        # 通过各自的编码器
        dino_patches = self.dino_featurizer(dino_flat)       # (B*N, num_patches, dino_dim)
        siglip_patches = self.siglip_featurizer(siglip_flat) # (B*N, num_patches, siglip_dim)
        ...
        # 在特征维度拼接（每个视角内拼接 dino 和 siglip 特征）
        combined = torch.cat([dino_patches, siglip_patches], dim=-1)  # (B, N, num_patches, total_dim)
    
        # 将 N 个视角的 patch 展平为序列
        visual_tokens = combined.view(B, -1, combined.shape[-1])      # (B, N * num_patches, total_dim)
